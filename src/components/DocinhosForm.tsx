...
export const DocinhosForm: React.FC<Props> = ({ data, onChange, onReset, onReview, onSwitch }) => {
  ...
  const update = (patch: Partial<ProposalData>) => {
    ...
    if (!Object.prototype.hasOwnProperty.call(patch, 'valorTotal')) {
      next.valorTotal = money(calculated(next)).replace('R$', '').trim();
    }
    onChange(next);
  };
  ...
  return <div className="proposal-form flex h-full flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white text-slate-800 shadow-sm">
    ...
    <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-5">
      ...
      <section><div className="mb-2 flex items-center justify-between"><h3 className="font-semibold">Docinhos <span className="text-xs font-normal text-stone-500">({sumQuantity} unidades)</span></h3><button type="button" onClick={() => update({ docinhosItens: [...items, { id: crypto.randomUUID(), nome: '', quantidade: 0, precoUnitario: 0 }] })} className="flex items-center gap-1 rounded-lg bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-800"><Plus className="h-3.5 w-3.5" /> Adicionar</button></div>
        <div className="space-y-2">{items.map((item, index) => <div key={item.id} className="grid grid-cols-[72px_minmax(0,1fr)_44px] items-end gap-2 sm:grid-cols-[1fr_80px_110px_32px]"><label className="col-span-3 sm:col-span-1">Nome do docinho<input aria-label={`Nome do docinho ${index + 1}`} placeholder="Ex: Brigadeiro" value={item.nome} onChange={e => updateItem(item.id, { nome: e.target.value })} className="form-input mt-1" /></label><label>Quantidade<input aria-label="Quantidade" type="number" min="0" step="1" value={item.quantidade || ''} onChange={e => updateItem(item.id, { quantidade: Math.max(0, Number(e.target.value)) })} className="form-input mt-1" placeholder="Qtd." /></label><label className="col-span-1">Preço unitário<input aria-label="Preço unitário" type="number" min="0" step="0.01" value={item.precoUnitario || ''} onChange={e => updateItem(item.id, { precoUnitario: Math.max(0, Number(e.target.value)) })} className="form-input mt-1" placeholder="R$" /></label><button type="button" onClick={() => update({ docinhosItens: items.filter(i => i.id !== item.id) })} className="flex min-h-10 w-full items-center justify-center text-stone-400 hover:text-rose-700" aria-label={`Excluir docinho ${index + 1}`}><Trash2 className="h-4 w-4" /></button></div>)}</div>
      </section>
      ...
      <section><div className="mb-2 flex items-center justify-between"><h3 className="font-semibold">Serviços</h3><button type="button" onClick={() => update({ servicos: [...services, { id: crypto.randomUUID(), descricao: '', valor: 0 }] })} className="flex items-center gap-1 rounded-lg bg-stone-100 px-3 py-2 text-xs font-semibold text-stone-700"><Plus className="h-3.5 w-3.5" /> Adicionar serviço</button></div>
        <div className="space-y-2">{services.map((service, index) => <div key={service.id} className="grid grid-cols-[minmax(0,1fr)_44px] items-end gap-2 sm:grid-cols-[1fr_110px_32px]"><label className="col-span-2 sm:col-span-1">Descrição do serviço<input placeholder="Ex: Entrega" value={service.descricao} onChange={e => updateService(service.id, { descricao: e.target.value })} className="form-input mt-1" /></label><label>Valor<input type="number" min="0" step="0.01" value={service.valor || ''} onChange={e => updateService(service.id, { valor: Math.max(0, Number(e.target.value)) })} className="form-input mt-1" placeholder="R$" /></label><button type="button" onClick={() => update({ servicos: services.filter(i => i.id !== service.id) })} className="flex min-h-10 w-full items-center justify-center text-stone-400 hover:text-rose-700" aria-label={`Excluir serviço ${index + 1}`}><Trash2 className="h-4 w-4" /></button></div>)}</div>
      </section>
